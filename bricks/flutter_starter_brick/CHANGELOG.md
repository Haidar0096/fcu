...
- The architecture lint plugin gains `apis_take_their_declared_client`: inside the composition root, a declared API must be built with exactly one named HTTP client, and it must be the one declared for it. Which client an API takes is the project's own answer, declared in `analysis_options.yaml` under `architecture_lint_rules: api_http_clients:`; the only shipped declaration is the sample jokes API on the public client, and an API a project does not declare is not checked. Twenty-six rules now.
- New dependency: `flutter_secure_storage`, pinned `^10.3.1`. The 10 line is the one that both resolves and runs clean here — on the 9 line a macOS run warns that the macOS plugin does not support Swift Package Manager, which a later Flutter turns into an error, and the 11 line cannot be resolved beside the app's `package_info_plus`.
